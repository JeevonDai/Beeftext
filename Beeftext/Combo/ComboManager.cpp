...
bool ComboManager::checkAndPerformComboSubstitution() {
	...
	if (triggersOnSpace) {
		bool cond = !currentText_.isEmpty() && currentText_.back().isSpace();
		if (cond) {
			currentText_ = "";
			this->onComboBreakerTyped();
			return false;
		}
		cond = (!currentText_.isEmpty()) &&
			(currentText_.back() == '`' || currentText_.back() == '~');
		Q_ASSERT(cond);
		if (!cond) return false;
		reverse_ = (currentText_.back() == '`');
		currentText_.chop(1);  // the last character is a space, and we want to
		// remove it before matching keywords
	}
	...
	std::sort(result.begin(), result.end(),
		[](SpCombo const& lhs, SpCombo const& rhs) -> bool {
	...
	res_ = result;
	ind_ = reverse_ ? 0 : result.size() - 1;

	if (result.empty()) {
		if (triggersOnSpace) {
			if (currentText_.size()) {
				eraseChars(currentText_.size() + 1);
			}
			this->onComboBreakerTyped();
		}
		return false;
	...
	SpCombo const combo = result[ind_];
	if ((!isBeeftextTheForegroundApplication()) &&
		(combo->performSubstitution(false) &&
			PreferencesManager::instance().playSoundOnCombo()) &&
		sound_)
...
void ComboManager::onReplaceTriggerShortcut() {
	if (!flag_) {
		return;
	}
	if (res_.empty()) {
	...
	if ((!isBeeftextTheForegroundApplication()) &&
		(combo->performSubstitution(tmp.size(), false) &&
			PreferencesManager::instance().playSoundOnCombo()) &&
		sound_)
		sound_->play();  // in Beeftext windows, substitution is disabled
	this->onComboBreakerTyped();
...
void ComboManager::onCharacterTyped(QChar c) {
	...
	if ((!prefs.useAutomaticSubstitution()) || (prefs.comboTriggersOnSpace() && (c != '`' && c != '~' && !c.isSpace())))
		return;
